...
export default function ProfilePage({ params }: { params: { username: string } }) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // Calculate stats
      const totalLikes = postsData?.reduce((sum, post) => sum + (post.likes?.[0]?.count || 0), 0) || 0;
      const totalComments = postsData?.reduce((sum, post) => sum + (post.comments?.[0]?.count || 0), 0) || 0;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <h2 className="text-2xl font-bold mb-8">Posts by {profile.full_name || profile.username}</h2>
